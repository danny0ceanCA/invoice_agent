...
def build_logic_system_prompt() -> str:
    ...
    return (
        ...
        "- School-year handling (SCUSD): school year N runs July 1 (N-1) through June 30 (N). When time_period.school_year is present, always apply invoice_date BETWEEN :start_date AND :end_date filters to every spend or hours query (students, providers, vendors, district, and comparisons). If a month is also provided, filter within that school-year window AND by the month. Do not ask for month/year clarification when school_year is given.\n"
        "- Canonical school-year spend by month:\n"
        ...
        "- Canonical school-year hours by month:\n"
        "SELECT LOWER(i.service_month) AS service_month,\n"
        ...
        "SCHOOL YEAR INTERPRETATION RULES:\n"
        "- Treat the district school year as July 1 through June 30.\n"
        "- \"this school year\" MUST map to:\n"
        "    invoice_date >= '2025-07-01' AND invoice_date <= '2026-06-30'\n"
        "  unless data clearly reflects a different active school-year boundary.\n"
        "- \"last school year\" shifts that window back one year.\n"
        "- For school-year questions:\n"
        "    • FILTER using invoice_date to respect district accounting.\n"
        "    • GROUP BY service_month for month-level summaries.\n"
        "- \"year-to-date\" must use:\n"
        "    • school-year YTD when question mentions \"school year\"\n"
        "    • calendar-year YTD (strftime('%Y', invoice_date)) otherwise.\n"
        "- When a question is ambiguous between school-year and calendar-year, do NOT guess:\n"
        "  set rows=null and put a short clarification note in IR.text.\n\n"
        ...
        "VENDOR VS CLINICIAN DISAMBIGUATION RULES:\n"
        "- Vendor names MUST come from vendors.name joined to invoices.vendor_id.\n"
        "- Clinician names MUST come from invoice_line_items.clinician.\n"
        "- Treat a name as a VENDOR when:\n"
        "    • name ends with 'services', 'care', 'supportive', 'agency', 'company', 'inc', 'llc'\n"
        "    • name contains phrases like 'supportive care', 'health services', 'action supportive care'\n"
        "- Treat a name as a CLINICIAN only when the question explicitly references:\n"
        "    \"clinician\", \"nurse\", \"l v n\", \"lvn\", \"hha\", \"aide\", \"health aide\",\n"
        "    \"therapist\", \"provider\", \"caregiver\"\n"
        "- Ambiguous names MUST NOT default to clinician; default to vendor or student unless clinician-only keywords appear.\n"
        "- Never match vendors using ili.clinician and never match clinicians using vendors.name.\n\n"
