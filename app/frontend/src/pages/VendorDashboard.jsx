...
export default function VendorDashboard() {
  ...
  const handleSubmit = useCallback(
      ...
      try {
        ...
        const queuedMessage =
          response.queue === "inline"
            ? `Upload processed immediately with status ${response.status}.`
            : `Upload received. Job ${response.job_id} is queued for processing.`;
        const detailedMessage = response.message
          ? `${queuedMessage} ${response.message}`
          : queuedMessage;
        setAlert({
          type: "success",
    ...
    [file, vendorId, invoiceDate, serviceMonth, invoiceCode, loadJobs, loadDashboard]
  );
